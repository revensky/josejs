...
import { RsaJwkParameters } from '../../jwk/rsa/rsa-jwk.parameters';
...
/**
 * Implementation of the RSASSA JSON Web Signature Backend.
 *
 * @see {@link https://www.rfc-editor.org/rfc/rfc7518.html#section-3.3 | RSASSA-PKCS1-v1_5 JWS Algorithms}
 * @see {@link https://www.rfc-editor.org/rfc/rfc7518.html#section-3.5 | RSASSA-PSS JWS Algorithms}
 */
export abstract class RSASSAJwsBackend extends JwsBackend {
  /**
   ...
   */
  protected abstract readonly padding: number;

  /**
   * JSON Web Key Key Type supported by the Backend.
   */
  protected readonly keyType: JwkKty = 'RSA';
   ...
   */
  public async sign(message: Buffer, jwk: JsonWebKey<RsaJwkParameters>): Promise<Buffer> {
    ...
    return await signAsync(this.hash, message, {
      key: createPrivateKey({ format: 'jwk', key: jwk.parameters }),
   ...
   */
  public async verify(signature: Buffer, message: Buffer, jwk: JsonWebKey<RsaJwkParameters>): Promise<boolean> {
    this.validateJsonWebKey(jwk, 'verify');

    return await verifyAsync(
      this.hash,
      message,
      { key: createPublicKey({ format: 'jwk', key: jwk.parameters }), padding: this.padding },
      signature,
    );
  }
}
