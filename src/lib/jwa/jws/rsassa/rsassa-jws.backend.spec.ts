...
Reflect.set(RSASSAJwsBackend.prototype, 'algorithm', 'ES256');
Reflect.set(RSASSAJwsBackend.prototype, 'hash', 'sha256');
Reflect.set(RSASSAJwsBackend.prototype, 'padding', constants.RSA_PKCS1_PADDING);

const publicParameters: RsaJwkParameters = {
  kty: 'RSA',
  n:
...
const privateParameters: RsaJwkParameters = {
  ...publicParameters,
  d:
...
describe('RSASSA JSON Web Signature Backend', () => {
  const backend: RSASSAJwsBackend = Reflect.construct(RSASSAJwsBackend, []);
  const message = Buffer.from('Super secret message.');

  describe('keyType', () => {
    it('should have "RSA" as its value.', () => {
      expect(backend['keyType']).toEqual<JwkKty>('RSA');
    });
  });

  describe('sign()', () => {
    it('should throw when signing with a public json web key.', async () => {
      const jwk = new JsonWebKey<RsaJwkParameters>(publicParameters);
  ...
  describe('verify()', () => {
    it('should return false when the signature does not match the message.', async () => {
      const jwk = new JsonWebKey<RsaJwkParameters>(privateParameters);
