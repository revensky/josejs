...
const jwkParameters: RSAJsonWebKeyParameters = {
  kty: 'RSA',
  n:
...
const invalidQs: any[] = [undefined, null, true, 1, 1.2, 1n, Symbol('foo'), Buffer, Buffer.alloc(1), () => 1, {}, []];
const invalidDPs: any[] = [undefined, null, true, 1, 1.2, 1n, Symbol('foo'), Buffer, Buffer.alloc(1), () => 1, {}, []];
...
describe('RSA JSON Web Key', () => {
  describe('constructor', () => {
    it.each(invalidKtys)('should throw when the provided "kty" is invalid.', (kty) => {
      expect(() => new RSAJsonWebKey({ ...jwkParameters, kty })).toThrowWithMessage(
    ...
    it.each(invalidNs)('should throw when the provided "n" is invalid.', (n) => {
      expect(() => new RSAJsonWebKey({ ...jwkParameters, n })).toThrowWithMessage(
        InvalidJsonWebKeyException,
    ...
    it.each(invalidEs)('should throw when the provided "e" is invalid.', (e) => {
      expect(() => new RSAJsonWebKey({ ...jwkParameters, e })).toThrowWithMessage(
        InvalidJsonWebKeyException,
    ...
    it.each(invalidDs)('should throw when the provided "d" is invalid.', (d) => {
      expect(() => new RSAJsonWebKey({ ...jwkParameters, d })).toThrowWithMessage(
        InvalidJsonWebKeyException,
    ...
    it.each(invalidPs)('should throw when the provided "p" is invalid.', (p) => {
      expect(() => new RSAJsonWebKey({ ...jwkParameters, p })).toThrowWithMessage(
        InvalidJsonWebKeyException,
    ...
    it.each(invalidQs)('should throw when the provided "q" is invalid.', (q) => {
      expect(() => new RSAJsonWebKey({ ...jwkParameters, q })).toThrowWithMessage(
        InvalidJsonWebKeyException,
    ...
    it.each(invalidDPs)('should throw when the provided "dp" is invalid.', (dp) => {
      expect(() => new RSAJsonWebKey({ ...jwkParameters, dp })).toThrowWithMessage(
        InvalidJsonWebKeyException,
    ...
    it.each(invalidDQs)('should throw when the provided "dq" is invalid.', (dq) => {
      expect(() => new RSAJsonWebKey({ ...jwkParameters, dq })).toThrowWithMessage(
        InvalidJsonWebKeyException,
    ...
    it.each(invalidQIs)('should throw when the provided "qi" is invalid.', (qi) => {
      expect(() => new RSAJsonWebKey({ ...jwkParameters, qi })).toThrowWithMessage(
        InvalidJsonWebKeyException,
        'Invalid json web key parameter "qi".',
      );
    });

    it('should return an instance of an rsa json web key.', () => {
      let jwk: RSAJsonWebKey = null!; // just to please the compiler
